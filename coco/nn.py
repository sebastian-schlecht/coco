...
class Network(object):
    ...
    def add(self, name, layer, grad_scale=1):
        ...
        if name in self._layers:
            raise AssertionError(
                "Layer names must be unique throughout the net. Layer %s already registered." % name)
    ...
    def load(self, filename, **tags):
        ...
        # If len doesn't match we don't care because we support partial
        # initialization
                    ...
                    if p.get_value().shape != v.shape:
                        raise AssertionError(
                            "Cannot set parameter %s of layer %s because their shapes don't match: %s vs. %s" % (
                                ...
                                self.last_layer=None
                                self.input=input
                                # In case some sub-class implements this, call
                                # it
                                self.init()
        ...
        """
                                ...
                                # Apply individual learning rate to all
                                ...
                                params=layer.get_params(
                                    unwrap_shared=unwrap_shared, **tags)
                                values=[p.get_value() for p in params]
                                data_store[key]=values
                                joblib.dump(data_store, filename,
                                            compress=compress)
