...
def resnet_50(input_var=None, n_classes=10):
    ...
    l = batch_norm(ConvLayer(l_in, num_filters=64, filter_size=(7, 7), stride=(2, 2), nonlinearity=rectify,
...
def resnet(input_var=None, n_classes=10, filter_config=(3, 4, 6, 3)):
    # create a residual learning building block with two stacked 3x3 convlayers as in paper
    ...
    for _ in range(1, filter_config[0]):
        l = residual_block(l)

    l = residual_block(l, increase_dim=True, projection=True)
    for _ in range(1, filter_config[1]):
        l = residual_block(l)

    l = residual_block(l, increase_dim=True, projection=True)
    for _ in range(1, filter_config[2]):
        l = residual_block(l)

    l = residual_block(l, increase_dim=True, projection=True)
    for _ in range(1, filter_config[3]):
        l = residual_block(l)
