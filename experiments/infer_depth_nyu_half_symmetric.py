...
from coco.architectures.depth import DepthPredictionScaffolder, SymmetricResidualDepth
...
def main():
    ...
    scaffolder = DepthPredictionScaffolder(SymmetricResidualDepth, train_processor, val_reader=val_processor, k=0.5)
    
    scaffolder.compile()
    Job.set_job_dir("/data/coco-jobs-relocated")
    scaffolder.fit(90, job_name="nyu_depth_half_symmetric_thesis", snapshot="/data/data/resunet_half_symmetric_thesis.npz", momentum=0.95,)
    scaffolder.save("/data/data/resunet_half_symmetric_thesis.npz")
